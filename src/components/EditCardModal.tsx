...
export function EditCardModal({ boardId, columnId, cardId, isOpen, onClose }: EditCardModalProps) {
  const { boards, updateCard } = useAppStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title.trim() || !card) return;

    updateCard(boardId, columnId, cardId, {
  ...
  const getDateInputValue = () => {
    ...
    const parsed = parseDate(formData.dueDate);
    if (!parsed) return '';
